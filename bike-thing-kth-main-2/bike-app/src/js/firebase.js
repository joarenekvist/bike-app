  ...
  const subscription = onSnapshot(userDocRef, (docSnapshot) => {
    if (docSnapshot.exists()) {
      ...
      if(rowData.userSettings){
        ...
        console.log("No entry found in database when fetching snapshot for the user setting field of ", state.user.email);
      }
...
async function updateUserSettingsInDatabase(state){
  ...
  try {
    ...
    await setDoc(userDocRef, {
      userSettings: {
        defaultComponentMileage: state.userSettings.defaultComponentMileage || 1000,
        receiveHumidityNotifications: state.userSettings.receiveHumidityNotifications,
        receiveTheftNotifications: state.userSettings.receiveTheftNotifications,
        receiveTireSwitchNotifications: state.userSettings.receiveTireSwitchNotifications
      }
    }, { merge: true });
    console.log("User settings updated successfully in database for: ", state.user.email);
  } catch (e) {
    console.error("Error updating user settings in firestore: ", e);
  }
}

export { auth, db, googleProvider, addUserToDatabase, subscribeToUserInDatabase, updateBikePartInformationInDatabase, updateUserSettingsInDatabase };
