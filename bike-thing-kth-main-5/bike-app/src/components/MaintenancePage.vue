                    ...
                    <v-card outlined hover class="maintenance-card" @click="viewComponentInfo(component.type)">
                      ...
                      <v-card-text>
                        ...
                        <div class="d-flex justify-space-between mt-1">
                          ...
                          <span class="text-caption">{{ formatMileage(component.currentMileage) }} / {{ formatMileage(component.maxLifespan) }} km</span>
                        </div>
                      </v-card-text>
                      <v-card-actions>
                        <v-btn 
            ...
            <v-row>
              <v-col v-for="(component, index) in filteredComponentsList" :key="index" cols="12" sm="6" md="4" lg="3">
                <v-card outlined hover @click="viewComponentInfo(component.name)" class="component-card">
                  ...
                  <v-card-subtitle>Typical lifespan: {{ formatMileage(component.defaultLifespan) }} km</v-card-subtitle>
                  <v-card-actions>
                ...
                <v-list dense>
                  ...
                  <v-list-item>
                    ...
                    <v-list-item-content>
                      ...
                      <v-list-item-subtitle>Around {{ formatMileage(selectedComponentInfo.defaultLifespan) }} km (or when health drops below 10%)</v-list-item-subtitle>
                    </v-list-item-content>
                  </v-list-item>
                </v-list>
      ...
      <v-dialog v-model="showMaintenanceDialog" max-width="500px">
        <v-card>
          ...
          <v-card-text>
            ...
            <p>Current health: <strong>{{ selectedMaintComponent?.healthPercentage || 0 }}%</strong></p>
            ...
            <v-form ref="maintenanceForm" v-model="maintenanceFormValid">
              ...
              <v-slider
                ...
                max="100"
                class="mt-6"
                :hint="`${selectedMaintComponent?.healthPercentage || 0}% + ${healthRestorePercentage}% = ${Math.min(100, (selectedMaintComponent?.healthPercentage || 0) + healthRestorePercentage)}% health`"
                persistent-hint
              ></v-slider>
            </v-form>
            ...
            >
              ...
              <span v-if="maintenanceType === 'Regular service'">Regular service typically adds about 50% health.</span>
              <span v-else-if="maintenanceType === 'Cleaning'">Cleaning typically adds about 20% health.</span>
              <span v-else-if="maintenanceType === 'Adjustment'">Adjustment typically adds about 30% health.</span>
              <span v-else-if="maintenanceType === 'Part replacement'">Part replacement typically adds about 90% health.</span>
              <span v-else-if="maintenanceType === 'Complete overhaul'">Complete overhaul restores component to 100% health.</span>
              <span v-else>The component's health will be restored according to the selected percentage.</span>
            </v-alert>
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
...
export default {
  ...
  watch: {
    ...
    // When maintenance type changes, update the health restoration percentage
    maintenanceType(newType) {
      // When maintenance type changes, update the health restoration percentage
      this.healthRestorePercentage = this.getHealthRestorationPercentage(newType);
    }
  },
  ...
  methods: {
    // Format mileage values to 2 decimal places
    formatMileage(value) {
    ...
    // Define restoration percentages based on maintenance type
    getHealthRestorationPercentage(maintenanceType) {
    ...
    // filter the components based on search query
    filterComponents() {
    ...
    openMaintenanceDialog(componentData) {
      ...
      this.healthRestorePercentage = this.getHealthRestorationPercentage('Regular service'); // Set default based on type
    ...
    completeMaintenance() {
      ...
      const currentHealth = this.selectedMaintComponent.healthPercentage || 0;
      
      // calculate the new health percentage based on maintenance type
      let newHealthPercentage;
      
  
      const restorationAmount = this.healthRestorePercentage;
      newHealthPercentage = Math.min(100, currentHealth + restorationAmount);
      ...
      const maintenanceRecord = {
        ...
        healthRestored: restorationAmount, // Store the actual amount restored
      ...
      this.showSuccessSnackbar(`Maintenance recorded: ${this.maintenanceType} (+${restorationAmount}% health)`);
    },
