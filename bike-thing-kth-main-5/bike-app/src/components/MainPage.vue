          ...
          <v-main class="mainContent">
            <div class="rightOfNavBar">
              ...
              <div class="dashboardGrid">
                ...
                />
                <WeatherNotificationWidget
                  v-if="showWeatherAlert"
                  class="widget left"
                  :weather="weatherData"
                  :minimized="weatherMinimized"
                  @acknowledged="weatherMinimized = true"
                  @cleared="showWeatherAlert = false"
                />
                <!--The parking widget is now more central to draw user focus-->
                <ParkingWidget class="widget center" @update-toggle="handleParkingToggle" :parked="parked"/>
...
import WeatherNotificationWidget from '@/widgets/WeatherNotificationWidget.vue';
...
export default {
  ...
  methods: {
    ...
    handleParkingToggle(newValue){//Moving or Parked
      this.$emit("handle-parking-toggle", newValue);
    },
    updateWeatherData(newWeather) {
    this.weatherData = newWeather;

    const temp = newWeather?.temperature;
    const wind = newWeather?.windspeed10m?.[0] || 0;
    const rain = newWeather?.precipitation || 0;

    const isBad = temp < 0 || temp > 35 || wind > 40 || rain > 50;

    this.showWeatherAlert = isBad;
    if (!isBad) {
      this.weatherMinimized = false;
    }
  },
} 
};
