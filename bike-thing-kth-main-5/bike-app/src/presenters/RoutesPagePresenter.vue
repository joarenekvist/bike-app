<template>
    <div>
      <RoutesPage @fetch-new-route="manualFetchNewRoute" :medals = "userMedals" @map-init="sendMapToModel" @send-coords-for-polylining="sendCoordsForPolylining" :cachedRoutes="cachedRoutes" :totalDistanceTravelled="totalDistanceTravelled"/>
    </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: { //values from store
        cachedRoutes() {
            return this.$store.getters.cachedRoutes || [];  //important to use an OR statement (||) in case its not yet defined. catches edge cases.
        },
        totalDistanceTravelled(){
          return this.$store.getters.totalDistanceTravelled || 0;  //important to use an OR statement (||) in case its not yet defined. catches edge cases.
        },
        userMedals(){
          return this.$store.getters.userMedals || [];
        }
    ...
    methods: {
      ...
      manualFetchNewRoute(){
        this.$store.dispatch("manualFetchNewRoute");
      }
    },
