...
export const store = createStore({
  state: {
    ...
    userMedals: [0], //array of ids (integers) corresponding to medals in MedalsWidget.
    parked: "Moving",
  ...
  getters: {
    ...
    userMedals: (state) => state.userMedals,
  ...
  mutations: {
    ...
    HANDLE_PARKING_SWITCH(state, newValue){
        state.parked = newValue;
        switchParking(state, newValue);
    ...
    MANUAL_FETCH_NEW_ROUTE(){
      console.log("deprecated manual fetch new route");
    },
  },
  actions: {
    ...
    manualFetchNewRoute({commit}){
      commit('MANUAL_FETCH_NEW_ROUTE');
    },

    handleParkingToggle({commit}, newValue){
      commit('HANDLE_PARKING_SWITCH', newValue);
    },
